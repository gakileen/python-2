...
def getRatingInfo(ratings):
    ...
    for line in ratings:
        rate = line.split(",")
        rates.append([int(rate[0]), int(rate[1]), int(float(rate[2]))])
    return rates
...
if __name__ == '__main__':
    ...
    movies_file = '/Users/ac/AI/data/MovieLens/ml-latest-small/movies.csv'
    ratings_file = '/Users/ac/AI/data/MovieLens/ml-latest-small/ratings.csv'

    # 获取所有电影的列表
    movies = getMovieList(movies_file)
    recommend_list, user_movie, items_movie, neighbors = recommendByUserFC(ratings_file, 50, 80)
